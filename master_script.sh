...
running_type=vehicleTest #hyperparameterSelection #hyperparameterOffline_distance #hyperparameterOffline_time #sparsityTest #vehicleTest #samplingHyperparameterSelection #samplingReduceFutureSelect #testmaxcapacity
...
do
	...
	do
		...
		do
			...
			#sbatch --job-name=i$ABBREVIATION$SPARSITY$VEHICLES --export=VEHICLES=$VEHICLES,OBJECTIVE=$OBJECTIVE,EXTENDED_HORIZON_ARRAY="${EXTENDED_HORIZON_ARRAY}",SPARSITY=$SPARSITY,HOUR=$HOUR,HEURISTIC_TIME_RANGE=$HEURISTIC_TIME_RANGE,DISTANCE_LOCATION=$DISTANCE_LOCATION,POLICY=$POLICY,NUM_INSTANCES=$NUM_INSTANCES run_bash_createInstances.cmd
			sleep 0.01	
			for EXTENDED_HORIZON in ${EXTENDED_HORIZON_ARRAY}
			do
				echo "run training"
				#sbatch --job-name=l$ABBREVIATION$SPARSITY$VEHICLES --export=OBJECTIVE=$OBJECTIVE,EXTENDED_HORIZON=$EXTENDED_HORIZON,VEHICLES=$VEHICLES,SPARSITY=$SPARSITY,HEURISTIC_TIME_RANGE_ARRAY_HYP="${HEURISTIC_TIME_RANGE_ARRAY_HYP}",DISTANCE_LOCATION_ARRAY_HYP="${DISTANCE_LOCATION_ARRAY_HYP}",MAXIMUM_CAPACITY_ARRAY_HYP="${MAXIMUM_CAPACITY_ARRAY_HYP}",STANDARDIZATION=$STANDARDIZATION,POLICY=$POLICY,HEURISTIC_TIME_RANGE=$HEURISTIC_TIME_RANGE,DISTANCE_LOCATION=$DISTANCE_LOCATION,HOUR=$HOUR,RUNNING_TYPE=$running_type,NUM_PERTURBATIONS=$NUM_PERTURBATIONS run_bash_training.cmd
				sleep 0.01
			done
		done

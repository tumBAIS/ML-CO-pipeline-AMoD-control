...
def get_learning_file(args, step):
    return args.learned_parameters_directory + f"Learning_{step}-{get_obj_str(args)}_{get_ext_horizon_str(args)}_{get_fleetSize_str(args)}_{get_perturbations(args)}"
...
def set_additional_args(args):
    ...
    args.training_instance_directory = args.training_instance_directory + f"_{args.policy}_{args.num_vehicles}_{args.sparsity_factor}_{args.objective}_{args.extended_horizon}/"
...
def save_training_data(args, edges, full_information_solution_edges, vehicleList, edge_features, look_up_grid, verticesList):
    def save(training_data):
        instance_output_file = open(args.training_instance_directory + f"TrainingInstance:{get_obj_str(args)}_{get_startDate_str(args)}_{get_endDate_str(args)}_"
                                                                       f"{get_sys_time_period_str(args)}_{get_ext_horizon_str(args)}_{get_instDate_str(args)}_{get_fleetSize_str(args)}", "wb")

        pickle.dump(training_data, instance_output_file)
        instance_output_file.close()

    missing_edges_vehicles_requests = [edge for edge in full_information_solution_edges["edgeVehiclesRequests_FI"] if edge not in edges["edges_vehicles_requests"]]
    missing_edges_requests_requests = [edge for edge in full_information_solution_edges['edgeRequestsRequests_FI'] if edge not in edges["edges_requests_requests"]]
    missing_edges_vehicles_artRebVertices = [edge for edge in full_information_solution_edges['edgeVehiclesArtRebVertices_FI'] if edge not in edges["edges_vehicles_artRebVertices"]]
    missing_edges_requests_artRebVertices = [edge for edge in full_information_solution_edges['edgeRequestsArtRebVertices_FI'] if edge not in edges["edges_requests_artRebVertices"]]
    missing_edges_artRebVertices_artCabVertices = [edge for edge in full_information_solution_edges['edges_artRebVertices_artCapVertices_FI'] if edge not in edges['edges_artRebVertices_artCapVertices']]
    ...
    if args.policy == "policy_CB":
        ...
        remove_from_capacity(full_information_solution_edges['edges_artRebVertices_artCapVertices_FI'], artRebVertices_idxs_to_capacity_cells_to_remove_from_vehicle+artRebVertices_idxs_to_capacity_cells_to_remove_from_request)
    ...
    def check_if_full_information_solution_is_feasible_solution():
        ...
        solution_edges = full_information_solution_edges["edgeVehiclesRequests_FI"] + full_information_solution_edges['edgeRequestsRequests_FI'] + full_information_solution_edges['edgeVehiclesArtRebVertices_FI'] + \
                         full_information_solution_edges['edgeRequestsArtRebVertices_FI'] + full_information_solution_edges['edges_artRebVertices_artCapVertices_FI']
        solution_edges_rebVertices_artCapVertices = copy.deepcopy(full_information_solution_edges['edges_artRebVertices_artCapVertices_FI'])
        training_edges = edges["edges_source_vehicles"] + edges["edges_vehicles_sink"] + edges["edges_requests_sink"] + edges["edges_artLocVertices_sink"] + \
                edges["edges_vehicles_requests"] + edges["edges_requests_requests"] + edges["edges_vehicles_artRebVertices"] + \
                edges["edges_requests_artRebVertices"] + edges["edges_artRebVertices_artCapVertices"]
        solution_edges.sort(key=lambda tup: tup[0])
        vehicleTrips = {}
        verticesIndexVehicleMapping = {}

        for solution_edge in solution_edges:
            if solution_edge in edges["edges_artRebVertices_artCapVertices"]:
                continue
    ...
    training_data = {"edges_source_vehicles": edges["edges_source_vehicles"],
                  "edges_vehicles_sink": edges["edges_vehicles_sink"],
                  "edges_requests_sink": edges["edges_requests_sink"],
                  "edges_artLocVertices_sink": edges["edges_artLocVertices_sink"],
                  "features_edges_vehicles_requests": edge_features["features_edges_vehicles_requests"],
                  "features_edges_requests_requests": edge_features["features_edges_requests_requests"],
                  "features_edges_vehicles_artRebVertices": edge_features["features_edges_vehicles_artRebVertices"],
                  "features_edges_requests_artRebVertices": edge_features["features_edges_requests_artRebVertices"],
                  "features_edges_artRebVertices_artCapVertices": edge_features["features_edges_artRebVertices_artCapVertices"],

...
parser.add_argument('--policy', type=str, help="specifies the policy", choices=['greedy', 'sampling', 'policy_SB', 'policy_CB', 'offline'], default="policy_SB")
...
parser.add_argument('--start_date', type=lambda s: datetime.strptime(s, '%Y-%m-%d %H:%M:%S'), help="start date of simulation | Example: 2020-01-14 00:00:00 (YYYY-MM-DD hh:mm:ss)", default="2015-01-26 09:00:00")

# determines the end of the simulation
parser.add_argument('--end_date', type=lambda s: datetime.strptime(s, '%Y-%m-%d %H:%M:%S'), help="end date of simulation | Example: 2020-01-14 00:00:00 (YYYY-MM-DD hh:mm:ss)", default="2015-01-26 10:00:00")
...
parser.add_argument('--instance_date', type=lambda s: datetime.strptime(s, '%Y-%m-%d %H:%M:%S'), help="start date of training instance | Example: 2020-04-01 00:00:00 (YYYY-MM-DD hh:mm:ss)", default="2015-01-26 09:12:00")
...
parser.add_argument('--fix_capacity', type=int, help="fix maximum number of capacities per cell", default=4)
...
parser.add_argument('--training_instance_directory', type=str, help="directory to save the instances for training", default="./training_instances/training_instances")

# sparsity factor
parser.add_argument('--sparsity_factor', type=float, help="factor to sparify request density", default=0.3)

# standardization
parser.add_argument('--standardization', type=int, help="standardizes all features between 0 and x", default=0)
...
parser.add_argument('--perturbed_optimizer', type=float, nargs='*', help="set n for perturbed optimizer; if n>0", default=[50.0, 1.0])
...
parser.add_argument('--max_iter', type=int, help="stopping criteria: stop after max_iter iterations", default=100)

# specify the directory to save the learned parameters and results from optimization
parser.add_argument('--learned_parameters_directory', type=str, help="directory to save all the learned parameters", default="./learned_parameters/learned_parameters")
...
parser.add_argument('--result_directory', type=str, help="directory to save all the results", default="./results/results")

### for testing
# set iteration to read in parameters from
parser.add_argument('--read_in_iterations', type=int, help="iteration to read in data for testing", default=None)

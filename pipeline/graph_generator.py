...
class GraphGenerator:
    ...
    def generate_graph(self, edges_source_vehicles, edges_vehicles_requests, edges_requests_requests, edges_vehicles_artRebVertices, edges_requests_artRebVertices, edges_artRebVertices_artCapVertices,
                       edges_vehicles_sink, edges_requests_sink, edges_artLocVertices_sink,
                       weights_source_vehicles, weights_vehicles_requests, weights_requests_requests, weights_vehicles_artRebVertices,
                       weights_requests_artRebVertices, weights_artRebVertices_artCapVertices, weights_vehicles_sink, weights_requests_sink, weights_artLocVertices_sink):

        print("     amount_edgeSourceVehicles:{}".format(len(edges_source_vehicles)))
        print("     amount_edgeVehicleRides:{}".format(len(edges_vehicles_requests)))
        print("     amount_edgeRidesRides:{}".format(len(edges_requests_requests)))
        print("     amount_edgeRidesSink:{}".format(len(edges_requests_sink)))
        print("     amount_edgeVehiclesSink:{}".format(len(edges_vehicles_sink)))
        print("     amount_edgeVehiclesArtificialRebalancingVertices:{}".format(len(edges_vehicles_artRebVertices)))
        print("     amount_edgeRidesArtificialRebalancingVertices:{}".format(len(edges_requests_artRebVertices)))
        print("     amount_edgeArtificialRebalancingVerticesArtificialCapacityVertices:{}".format(len(edges_artRebVertices_artCapVertices)))
        print("     amount_edgesartLocVerticesSink:{}".format(len(edges_artLocVertices_sink)))
        ...
        def enter_dummy_edges(edges, weights):
            ...
            for weight, edge in zip(weights, edges):
                if edge[0] not in self.artificial_dummy_edge_mapping.keys():
                    self.artificial_dummy_edge_mapping[edge[0]] = self.sink_index
        ...
        self.original_sink_index = self.sink_index
        if self.args.policy == "policy_CB":
            self.artificial_dummy_edge_mapping = {}
            edges_requests_requests, weights_requests_requests = enter_dummy_edges(edges_requests_requests, weights_requests_requests)
            edges_requests_artRebVertices, weights_requests_artRebVertices = enter_dummy_edges(edges_requests_artRebVertices, weights_requests_artRebVertices)
            edges_requests_sink, weights_requests_sink = enter_dummy_edges(edges_requests_sink, weights_requests_sink)

            edges_vehicles_sink = [(edge[0], self.sink_index) for edge in edges_vehicles_sink]
            edges_requests_sink = [(edge[0], self.sink_index) if edge[0] not in self.artificial_dummy_edge_mapping.keys() else (edge[0], edge[1]) for edge in edges_requests_sink]
            edges_artLocVertices_sink = [(edge[0], self.sink_index) for edge in edges_artLocVertices_sink]

        # combine all edges and weights
        edgesList = edges_source_vehicles + edges_vehicles_requests + edges_requests_requests + edges_vehicles_artRebVertices + edges_requests_artRebVertices + \
                    edges_artRebVertices_artCapVertices + edges_vehicles_sink + edges_requests_sink + edges_artLocVertices_sink
        weightsList = weights_source_vehicles + weights_vehicles_requests + weights_requests_requests + weights_vehicles_artRebVertices + \
                      weights_requests_artRebVertices + weights_artRebVertices_artCapVertices + weights_vehicles_sink + weights_requests_sink + weights_artLocVertices_sink
    ...
    def deleting_sink_source_edges(self, edges, weights=None):
        remaining_edges = []
        remaining_weights = []
        for edge, weight in zip(edges, weights if weights is not None else [0] * len(edges)):
            # we ignore edges from the source
            if edge[0] == 0:
                continue
            # we ignore edges to the sink
            elif edge[1] == self.sink_index:
                continue
            remaining_edges.append(edge)
            remaining_weights.append(weight)
        if self.args.policy == "policy_CB":
            remaining_edges, remaining_weights = self.resolve_dummy_edges_mapping(remaining_edges, remaining_weights)
        return remaining_edges, remaining_weights
    ...
    def resolve_dummy_edges_mapping(self, edges, weights=None):
        ...
        for edge, weight in zip(edges, weights if weights is not None else [0] * len(edges)):
            ...
            if edge[1] == self.sink_index:
                edge = (edge[0], self.original_sink_index)
    ...
    def get_full_information_solution_edges(self, edges, vehicleList, requestList, full_information_artRebVerticesList, artRebVerticesList):
        ...
        return {"edgeVehiclesRequests_FI": [tuple(x) for x in edges_vehicles_requests[["vertex_idx_x", "vertex_idx_y"]].values.tolist()],
                ...
                "edges_artRebVertices_artCapVertices_FI": [tuple(x) for x in edges_artRebVertices_artCapVertices]}
